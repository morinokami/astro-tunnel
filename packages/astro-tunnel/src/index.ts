	...
	return {
		...
		hooks: {
			"astro:config:setup": ({ addDevToolbarApp, config, updateConfig }) => {
				...
				if (
					config.vite.server?.allowedHosts !== true &&
					!config.vite.server?.allowedHosts?.includes(".trycloudflare.com")
				) {
					// https://vite.dev/config/server-options#server-allowedhosts
					if (Array.isArray(config.vite.server?.allowedHosts)) {
						config.vite.server.allowedHosts.push(".trycloudflare.com");
					} else {
						config.vite.server = {
							...config.vite.server,
